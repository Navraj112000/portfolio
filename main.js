...
function initMarquee(){
  ...
  if(!track) return;
  const marqueeItems = ['UI/UX Designer','UI/UX Developer','Web Design','App Design','Web Development','Brand Strategy','Product Design'];
...
const SERVICES = [
  {icon:'🎨', title:'UI/UX Design', short:'Interfaces that are easy to use and consistent across every screen and state.',
    includes:['Wireframes & user flows','High-fidelity UI screens','Design system / component library']},
  {icon:'🧭', title:'UX Research', short:'User interviews and testing that ground design decisions in real behavior.',
    includes:['User interviews','Usability testing','Journey & funnel mapping']},
  {icon:'📱', title:'App Design', short:'Mobile-first product design built for clarity, speed, and adoption.',
    includes:['iOS & Android UI kits','Prototype for user testing','Handoff-ready specs']},
  {icon:'🔗', title:'Web Design', short:'Marketing and product sites that convert without sacrificing craft.',
    includes:['Landing pages','Responsive layouts','Conversion-focused copy structure']},
  {icon:'💡', title:'Brand Strategy', short:'Visual identity systems that hold up across every touchpoint.',
    includes:['Color & type systems','Logo & mark refinement','Brand guideline docs']},
  {icon:'⚙️', title:'Web Development', short:'Front-end builds that translate design files into shipped product.',
    includes:['HTML/CSS/JS builds','React component builds','QA against design specs']},
];
...
function renderProjectGrid(targetId, filter){
  ...
  const list = (!filter || filter === 'All') ? PROJECTS : PROJECTS.filter(p => p.tag === filter);
